...
class TimeTableReport(models.TransientModel):
    ...
    def gen_time_table_report(self):
        data = self.read(
            ['start_date', 'end_date', 'class_id', 'state',
             'faculty_id'])[0]
        if data['state'] == 'student':
            time_table_ids = self.env['ems.timetable'].search(
            ...
            return self.env['report'].get_action(
                self, 'emsprime_timetable.report_timetable_student_generate',
                data=data)
        else:
            teacher_time_table_ids = self.env['ems.timetable'].search(
                [('start_datetime', '>', data['start_date'] + '%H:%M:%S'),
                 ('end_datetime', '<', data['end_date'] + '%H:%M:%S'),
            ...
            return self.env['report'].get_action(
                self, 'emsprime_timetable.report_timetable_teacher_generate',
                data=data)
